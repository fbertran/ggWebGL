...
`ggWebGL` and shows compact CRAN-safe usage patterns. The examples use in-memory
data only and avoid file output.

The code examples construct `ggplot2`-style WebGL layers and, when evaluated,
convert them into browser-side WebGL htmlwidgets with `ggplot_webgl()`.
Evaluation is disabled during CRAN, package checks, and CI unless explicitly
...
The same renderer can also consume a renderer-ready specification. This is
useful when data have already been transformed into primitive payloads:
...
The table below uses conservative status labels. `Stable` means the API is part
of the core two-dimensional renderer workflow. `Experimental` means the API is
exported and tested, but the rendering contract or interaction details may still
evolve. `Metadata-only` means metadata can be serialized without implying full
runtime parity with every `ggplot2` feature. These labels reflect API maturity,
test coverage, and rendering-contract stability; they are not simply an export
list.
...
| Count and bin geoms | `geom_bar_webgl()`, `geom_histogram_webgl()`, `geom_bin2d_webgl()` | Experimental | Counts and bins are computed by `ggplot2`; WebGL serialization is tested but newer than the core point/line/raster path. |
| Curves and contours | `geom_freqpoly_webgl()`, `geom_density_webgl()`, `geom_density2d_webgl()`, `geom_contour_webgl()` | Experimental | Rendered as line/path primitives; filled contours are not claimed. |
| Ranges and summaries | `geom_linerange_webgl()`, `geom_errorbar_webgl()`, `geom_pointrange_webgl()`, `geom_crossbar_webgl()`, `geom_boxplot_webgl()` | Experimental | Uses built summary columns and point/segment/rectangle primitives. |
| Filled regions | `geom_ribbon_webgl()`, `geom_area_webgl()`, `geom_polygon_webgl()`, `geom_violin_webgl()` | Experimental | Filled regions use simple polygon/ribbon-style rendering; complex polygon topology is limited. |
| Raster and grids | `geom_raster_webgl()` | Stable | Intended for regular raster-like cell displays. |
| Text annotations | `geom_text_webgl()`, `geom_label_webgl()` | Metadata-only | Text and label layers are overlay metadata, not WebGL glyph rendering. |
| Rug annotations | `geom_rug_webgl()` | Stable | Rugs use segment primitives. |
| Meshes and surfaces | `geom_mesh_webgl()`, `geom_surface_webgl()` | Experimental | Browser-side WebGL mesh and structured-grid surface rendering. |

# Core 2D Layers

Points, ordered paths, sorted lines, and plain segments cover the most common
two-dimensional display cases.
...
`geom_line_webgl()` keeps the usual line semantics, while `geom_path_webgl()`
...
Frequency polygons, density curves, two-dimensional density contours, and
regular contour lines all serialize to line/path primitives. The statistical
work remains with `ggplot2`.
...
`geom_density2d_webgl()` uses `ggplot2`'s two-dimensional density statistic,
which requires `MASS` at render time. The chunk is skipped when that optional
runtime dependency is unavailable.

```{r density2d-contours, eval = ggwebgl_eval_examples && requireNamespace("MASS", quietly = TRUE)}
p <- ggplot(mtcars, aes(wt, mpg)) +
...
Range and summary geoms combine segment, point, and rectangle primitives.

```{r linerange-pointrange}
...
Ribbon, area, and simple polygon layers are filled-region APIs. They are useful
for compact displays, but complex polygon topology such as holes or
self-intersections should be treated as outside the current core contract.
...
Raster layers are intended for regular cell displays. Text and label layers are
overlay metadata, not full WebGL glyph rendering.
...
If label backgrounds are needed, `geom_label_webgl()` serializes label metadata
...
Fixed-scale facets are supported across the primitive families. Free-scale
facets are serialized conservatively as metadata unless panel-local scaling is
available for the layer combination being rendered.
...
Unsupported geoms remain visible to the extraction layer as unsupported metadata
rather than being silently claimed as WebGL primitives. When a plot needs exact
`ggplot2` parity for an unsupported layer, keep that layer in a static ggplot or
replace it with one of the supported WebGL primitives above.

...
runtime parity with every `ggplot2` feature.
...
| Count and bin geoms | `geom_bar_webgl()`, `geom_histogram_webgl()`, `geom_bin2d_webgl()` | Stable | Counts and bins are computed by `ggplot2`. |
| Curves and contours | `geom_freqpoly_webgl()`, `geom_density_webgl()`, `geom_density2d_webgl()`, `geom_contour_webgl()` | Stable | Rendered as line/path primitives; filled contours are not claimed. |
| Ranges and summaries | `geom_linerange_webgl()`, `geom_errorbar_webgl()`, `geom_pointrange_webgl()`, `geom_crossbar_webgl()`, `geom_boxplot_webgl()` | Stable | Uses built summary columns and point/segment/rectangle primitives. |
...
| Text annotations | `geom_text_webgl()`, `geom_label_webgl()`, `geom_rug_webgl()` | Metadata-only / Stable | Text and label layers are overlay metadata, not WebGL glyph rendering; rugs use segment primitives. |

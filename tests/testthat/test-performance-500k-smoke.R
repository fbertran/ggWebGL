...
test_that("500k performance smoke script exposes manual and browser helpers", {
  ...
  expect_match(text, "run_manual_1m_progressive_performance_smoke_test <- function", fixed = TRUE)
  ...
  expect_match(text, "point_count = 1000000L", fixed = TRUE)
  expect_match(text, "density_splat", fixed = TRUE)
  expect_match(text, "ggwebgl_transport", fixed = TRUE)
  ...
  expect_match(text, "__ggwebgl_transport_metrics", fixed = TRUE)
...
test_that("500k performance smoke data is deterministic and reduced widgets build", {
  ...
  expect_equal(widget$x$render$transport$compact_layers, 0L)
})

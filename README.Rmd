...
stable current scope covers core widget/specification construction, point, line,
raster, fixed-scale facet, pan/zoom/hover, `Shiny` output, and static export
helpers. Vector, mesh, surface, trajectory, timeline, brush/lasso, transport,
and structured 3D view support are implemented public APIs but remain
...
Status labels are conservative. `Stable` means the API is part of the core
package path; `Experimental` means it is exported and tested but still subject
to renderer-contract refinement; `Optional extension` means the feature depends
on a suggested downstream package; `Metadata-only` means the package records
information without claiming full rendering support; `Deferred` means the item
is intentionally out of the current core package.

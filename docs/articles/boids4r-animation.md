...
`boids_simulation` object and animated by the `ggWebGL` timeline. Points
show boids, arrows show current velocity, and obstacles are drawn as
rings. Optional trails are a recent history window, not the current boid
count.

...
Add a violin layer tagged for the `ggWebGL` renderer. Density estimation
and scaling are computed by
[`ggplot2::stat_ydensity`](https://ggplot2.tidyverse.org/reference/geom_violin.html);
the renderer converts each built violin group to a mesh-backed filled
strip. Quantile guide lines and exact stroke parity with
...
- stat:
  ...
  Statistical transformation to use. Defaults to `"ydensity"` so
  `ggplot2` computes violin density and scaling before WebGL
  serialization.
...
- ...:
  ...
  including
  [`ggplot2::stat_ydensity()`](https://ggplot2.tidyverse.org/reference/geom_violin.html)
  parameters such as `bw`, `adjust`, `kernel`, `n`, and quantile-related
  arguments, plus static aesthetics.

...
Add a filled ribbon layer tagged for the `ggWebGL` renderer. The
renderer consumes `ggplot2`-built `x`, `ymin`, and `ymax` values and
draws each group/run as a filled triangle strip.

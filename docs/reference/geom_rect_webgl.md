...
Add a rectangle layer tagged for the `ggWebGL` renderer. Boundaries are
taken from the data built by `ggplot2`, so `xmin`, `xmax`, `ymin`, and
`ymax` follow the same setup rules as
